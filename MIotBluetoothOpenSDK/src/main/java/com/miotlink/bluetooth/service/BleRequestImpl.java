...
import com.miotlink.bluetooth.callback.wrapper.ConnectWrapperCallback;
...
import com.miotlink.bluetooth.callback.wrapper.WriteWrapperCallback;
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    ...
                    if (Ble.options().getMtu()>20){
                        setMtu(device.getAddress(),Ble.options().getMtu());
                        return;
                    }
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status){
            if (gatt != null && gatt.getDevice() != null) {
                ...
                if (null != mtuWrapperCallback){
                    ...
                    BluetoothGatt bluetoothGatt = getBluetoothGatt(gatt.getDevice().getAddress());
                    if (null != bluetoothGatt){
                        BleLog.d(TAG, "trying to start service discovery");
                        bluetoothGatt.discoverServices();
                    }
