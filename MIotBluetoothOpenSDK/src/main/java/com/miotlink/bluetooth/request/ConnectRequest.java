...
import com.miotlink.bluetooth.queue.retry.RetryDispatcher;
...
public class ConnectRequest<T extends BleDevice> implements ConnectWrapperCallback<T> {
    ...
    public void connect(List<T> devices, final BleConnectCallback<T> callback) {
        dispatcher.excute(devices, device -> connect(device, callback));
    }
    ...
    public T getConnectedDevice(String macCode) {
        return  connectedDevices.get(macCode);
    }
