...
public class CRC16Utils {
	...
	public static byte[] getSendBuf(String toSend){
		...
		CRC16Utils crc16 = new CRC16Utils();
	...
	public static byte[] getCRC(String str) {
		...
		String hexStr = bytesToHexString(bytes, bytes.length);
		byte[] buf = CRC16Utils.getSendBuf(hexStr);
		String crc = CRC16Utils.getBufHexStr(buf).toLowerCase();
		System.out.println(crc);
		return buf;

	}
	public static String getCRCValue(byte[] bytes) {
		String hexStr = bytesToHexString(bytes, bytes.length);
		byte[] buf = CRC16Utils.getSendBuf(hexStr);
		String crc = CRC16Utils.getBufHexStr(buf).toLowerCase();
		return crc;
	...
	public static int  getCrcMaxLen(String crc){
		int max=0;
		try{
			max=Integer.parseInt(CRC16Utils.getCRC16Max(crc), 16);
		}catch (Exception e){}

		return max;
	}
	public static int  getCrcMinLen(String crc){
		int max=0;
		try{
			max=Integer.parseInt(CRC16Utils.getCRC16Min(crc), 16);
		}catch (Exception e){}
